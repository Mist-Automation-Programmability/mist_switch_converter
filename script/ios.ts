...
function process_config(folder: string, filename: string): Promise<boolean> {
...
function process_vlans(folder: string, filename: string): Promise<boolean> {
    return new Promise((resolve) => {
        const readInterface = readline.createInterface({
            input: fs.createReadStream(path.join(folder, filename))
        });
...
function read_vlans(folder: string, files: string[]): Promise<boolean> {
    return new Promise((resolve) => {
        var i = 0;
        files.forEach((filename: string) => {
            console.log(filename);
            process_vlans(folder, filename).then((res) => {
                if (!res) console.error("Error when reading VLANs list from " + filename)
                i++;
                if (i == files.length) {
                    resolve(true);
                }
...
function read_config(folder: string, files: string[]): Promise<boolean> {
    return new Promise((resolve) => {
        var i = 0;
        files.forEach((filename: string) => {
            console.log(filename);
...
function start(folder: string, out_file: string) {
    ...
    read_vlans(folder, files).then((res) => {
        read_config(folder, files).then((res) => {
            ...
            fs.writeFile(out_file, JSON.stringify(mist_config.mist_template), (err) => {
                ...
                else console.log("Template saved in " + out_file);
            })
        })
    })
}
 ...
 * ENTRY POINT
 */
const mist_config = new IosParser();
var config_folder = undefined;
var out_file = "template.json";
var show_usage: boolean = false;
process.argv.forEach((value: string) => {
    var param_key: string = value.split("=")[0];
    var param_val: string = value.split("=")[1];
    switch (param_key) {
        case "--folder":
...
if (config_folder) start(config_folder, out_file);
else if (!show_usage) {
