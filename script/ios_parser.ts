...
export class IosParser {
    ...
    private parse_syslog(syslog_line: string) {
        ...
        if (transport_index > -1) syslog_proto = config[syslog_proto + 1]
        if (port_index > -1) syslog_port = config[syslog_port + 1]
    ...
    private parse_interface(interface_config: string[]) {
        ...
        var profile_configuration: object = {}
